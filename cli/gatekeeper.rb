...
class Gatekeeper < Thor
  ...
  VAULT_BUCKET_NAME = "keyshare-vault"
  ...
  def add(key, credential)
      ...
      response = @client.put_object(bucket: VAULT_BUCKET_NAME, key: key, body: credential)
      puts "INFO: Successfully added '#{key}' to #{VAULT_BUCKET_NAME}."
  end
  ...
  def get(key)
    begin
      response = @client.get_object(bucket: VAULT_BUCKET_NAME, key: key).body.read
      if options[:decrypt]
      ...
      puts "ERROR: Able to connect to S3, but specified key '#{key}' does not exist in S3 bucket '#{VAULT_BUCKET_NAME}'."\
      " You can add one using the #add method."
    rescue Aws::S3::Errors::NotFound
      puts "ERROR: S3 bucket '#{VAULT_BUCKET_NAME}' does not exist on this account/region!"\
      " Either you have misconfigured your AWS settings, or you have not created a"\
      " root-level '#{VAULT_BUCKET_NAME}' bucket in S3. You can do so now with the #create_vault command."
    end
  end

  desc "create_vault", "Create a '#{VAULT_BUCKET_NAME}' bucket on S3 with default configuration which acts as your keyring"
  def create_vault
    begin
      resp = @client.create_bucket({
        ...
        puts "INFO: Successfully created S3 bucket '#{VAULT_BUCKET_NAME}'! You can"\
        " now add a key-value credential pair using the #add command."
    rescue Aws::S3::Errors::BucketAlreadyOwnedByYou
      puts "WARNING: Bucket '#{VAULT_BUCKET_NAME}' already exists for this account in"\
  ...
  def key_exists?(bucket_name = VAULT_BUCKET_NAME, key)
    begin
      return @client.head_object({ bucket: VAULT_BUCKET_NAME, key: key }).successful?
    rescue Aws::S3::Errors::NotFound
      return false
    end
  end
